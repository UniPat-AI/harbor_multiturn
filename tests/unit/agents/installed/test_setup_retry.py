...
class RetryTestAgent(BaseInstalledAgent):
    def __init__(self, logs_dir: Path, install_script_path: Path):
    ...
    def create_run_agent_commands(self, instruction: str) -> list[ExecInput]:
        return [ExecInput(command="echo ready")]
...
def _install_exec_calls(mock_environment) -> list:
    return [
        ...
        if call.kwargs.get("command") == "bash /installed-agent/install.sh"
    ]
